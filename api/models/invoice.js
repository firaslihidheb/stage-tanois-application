...
class Invoice {
    ...
    addInvoice(req, res) {
        try {
            ...
            assert(req.body.Client_id, "Client Name is required");
            ...
            assert(req.body.contracts, "Please select atleast one contract");

            const columnList = ['inv_number', 'Client_id', 'inv_date', 'due_date', 'total_amt', 'due_amt', 'user_id'];
            const columnValuesList = ['?', '?', '?', '?', '?', '?', '?'];
            const columnValues = [req.body.inv_number, req.body.Client_id, req.body.inv_date, req.body.due_date, req.body.total_amt, req.body.due_amt, req.user_id];
            ...
            connectionPool.query(`INSERT INTO invoicer.invoice (${columnList.join(', ')}) VALUES 
                ...
                if (error) {
                    ...
                    let columnValuesStr = "";
                    req.body.contracts.forEach(data => {
                        columnValuesStr += ` (${result.insertId},${data.contract_id},${data.quantity},${data.rate}),`
                    });
                    columnValuesStr = columnValuesStr.slice(0, -1);
                    connectionPool.query(`INSERT INTO invoicer.invoice_contract (invoice_id,contract_id,quantity,rate) VALUES 
                                    ${columnValuesStr};`, function (error, result, fields) {
                        if (error) {
                            if (error.code === 'ER_DUP_ENTRY') {
                                res.status(422).send({
                                    status: "error",
                                    message: "Invoice with this Contract already exists"
                                });
                            } else {
                                res.status(500).send({
                                    status: "error",
    ...
    getInvoices(req, res) {
        try {
            ...
            connectionPool.query(`SELECT cst.name Client_name,
                                         inv.id,
                                         inv.due_amt,
                                         inv.inv_number,
                                         inv.inv_date,
    ...
    getInvoice(req, res) {
        try {
            ...
            connectionPool.query(`SELECT inv.*, ip.*, pr.name contract_name, cst.name Client_name, cst.email Client_email, cst.company_name Client_company_name, usr.mobile user_mobile,usr.email user_email,usr.name user_name,usr.company_name user_company_name,usr.company_address user_company_address
                                  FROM invoicer.invoice inv, invoicer.invoice_contract ip, invoicer.contract pr, invoicer.client cst, user usr
                                  where inv.id = ip.invoice_id and pr.id = ip.contract_id and inv.Client_id = cst.id and usr.id = inv.user_id and  inv.id = ? and inv.user_id = ?;`,
