...
class User {
    ...
    addUser(req, res) {
        ...
        try {
            ...
            assert(req.body.lastname, 'Mobile is required');
            ...
            connectionPool.query(`INSERT into user(name, lastname, password, email,qualification,type_id) VALUES(?,?,?,?,?,?)`, [
                name, lastname, hashedPassword, email,qualification, type_id], function (error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
    ...
    loginUser(req, res) {
        try {
            ...
            connectionPool.query(`SELECT id,password FROM user where lastname = ?`, username,
  ...
  getUsers(req, res){
    try {
        ...
        connectionPool.query(`SELECT * FROM user where user_id = ?`, req.user_id,
            function(error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
...
getUser(req, res){
    try {
        ...
        connectionPool.query(`SELECT * FROM user where id = ? and user_id = ?`,
