    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR){
            if(jqXHR.status === 200){
                ...
                data.data.forEach(i=>{
                    $('#Client_id').append(`<option value="${i.Client_id}">${i.name}</option>`);
                })
            }
            else if(jqXHR.status === 204){
                $('#Client_id').append(`<option value="">No client exist. Add new client via Client Tab above</option>`);
            }
        },
        error: function(jqXHR, textStatus, errorThrown){
...
(() => {
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR){
            if(jqXHR.status === 200){
                contractsArr = data.data;
                $('#contract_id').append(`<option value="">Select Contract</option>`);
                addContractsToSelect(1);
            }
            else if(jqXHR.status === 204){
                $('#contract_id').append(`<option value="">No Contract exist. Add new client via Contract Tab above</option>`);
            }
        },
        error: function(jqXHR, textStatus, errorThrown){
            console.log(jqXHR.responseJSON);
...
function addContractsToSelect(row_id){
    contractsArr.forEach(i=>{
        $('.contract.'+row_id).append(`<option value="${i.id}">${i.name}</option>`);
    })
}
$(document).ready(function () {
    ...
    $(document).on('submit', '.add-invoice-form', function () {
        ...
        let contracts = [];
        contracts.push({
            "contract_id": $('select[name="contract_id"]').val().trim(),
            "quantity": $('input[name="quantity"]').val().trim(),
            "rate": $('input[name="rate"]').val().trim()
        });

        let form_data = {inv_number, Client_id, inv_date, due_date, total_amt, due_amt,contracts};
    ...
    $(document).on("click", ".add_new_invoice_contract", function () {
        const count = $(".contract_parent > div").length + 1;
        const html = `<div class="row ${count} m-t-15"> <div class="col-md-6 "> <label>Name</label> <select class="form-control contract ${count}" required> <option>Select Contract</option> </select> </div><div class="col-md-1 "> <label>Quantity</label> <input type="number" step="1" min="1" max="9999" class="form-control quantity ${count}" placeholder="Qty" name="quantity" required> </div><div class="col-md-2 "> <label>Rate (in $)</label> <input type="number" step="0.01" min="0.01" max="9999" class="form-control rate ${count}" placeholder="Rate" name="rate" required> </div><div class="col-md-2 "> <label>Amount (in $)</label> <div></div></div><div class="col-md-1 remove_contract_row" data-rowid="${count}"><i class="fa fa-times" aria-hidden="true" ></i></div></div>`;
        $(".contract_parent").append(html);
    });
